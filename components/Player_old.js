...
const Player = props => {
    useEffect(()=>{
        ...
        const buttonPip = document.getElementById('pip-button');
        const fullscreenIcons = buttonFullscreen.querySelectorAll('use');
        ...
        if (video.paused) {
            buttonPlay.setAttribute('data-title', 'Play (k)');
        } else {
            buttonPlay.setAttribute('data-title', 'Pause (k)');
        }
        }
        ...
        function formatTime(timeInSeconds) {
            try{
                // console.log('rawTime:'+timeInSeconds);
        ...
        const seek = document.getElementById('seek'); // remove
        ...
        buttonVolume.setAttribute('data-title', 'Mute (m)');

        if (video.muted || video.volume === 0) {
            volumeMute.classList.remove('hidden');
            buttonVolume.setAttribute('data-title', 'Unmute (m)');
        ...
        if (document.fullscreenElement) {
            buttonFullscreen.setAttribute('data-title', 'Exit full screen (f)');
        } else {
            buttonFullscreen.setAttribute('data-title', 'Full screen (f)');
        }
        }
        ...
        if (!('pictureInPictureEnabled' in document)) {
            buttonPip.classList.add('hidden');
        }
        });
    ...
    return(
        <>
            <div className="container">
                ...
                <video controls={false} className="video" id="video" preload="metadata" poster="poster.jpg">
                    <source src={url1} type="video/mp4"></source>
                </video>
